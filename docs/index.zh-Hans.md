...
VRC Stocking Generator 是一个 Unity 编辑器扩展，可以从头像身体生成丝袜。它可以制作膝上袜、连裤袜、连体裤袜、手臂丝袜，也可以使用排除碰撞体制作自定义裁剪形状。

建议安装 Modular Avatar 和 lilToon。如果未安装 Modular Avatar，将不会配置 Blendshape Sync 和 Merge Armature。如果未安装 lilToon，将无法加载预设使用的材质。

## 基本使用方法 {#basic-usage}

1. 从 `Tools > Stocking Generator` 打开工具。
...
??? note "制作方法"
    ...
    将 `遮罩模式` 设置为 `膝上袜`，然后根据需要调整 `丝袜高度偏移`。
...
??? note "制作方法"
    ...
    启用 `连体裤袜`。如果想在颈部周围裁剪遮罩，请使用 `颈部遮罩`。启用膝上袜、连裤袜或手臂丝袜边界时，边界区域会被加深。

![](img/pose_default_fullbody.png)

- 左右高度不同，或只生成单侧膝上袜/手臂丝袜

??? note "制作方法"

    启用 `分开左右高度偏移`，然后分别调整左右两侧。如果只想生成单侧，请关闭不需要的一侧，例如 `右侧膝上袜`、`左侧膝上袜`、`右臂丝袜` 或 `左臂丝袜`。
...
??? note "制作方法"
    ...
    启用 `连体裤袜`，然后启用 `颈部遮罩`。在层级中添加一个 GameObject，并在其子对象中添加球体、胶囊和盒形碰撞体。
        ...
        <table>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td><code>无</code> 不生成腿部丝袜区域。<code>膝上袜</code> 生成膝上袜遮罩。<code>连裤袜</code> 生成下半身连裤袜风格遮罩。</td>
            </tr>
            <tr>
            ...
            <tr>
              <td><code>左侧膝上袜</code> / <code>右侧膝上袜</code></td>
              <td>分别启用或禁用左右两侧的膝上袜生成。<code>遮罩模式</code> 为 <code>膝上袜</code> 时显示。</td>
            </tr>
            <tr>
...
2. 添加子对象，在子对象上添加碰撞体，并放置到想要移除的区域。
3. 在工具中启用 `排除碰撞体`。
4. 将父对象指定到 `根对象`。
